...
class Classify_Dataset:
    ...
    def init_template(self):
        self.cause_level2 = '["Declared dead by the court", ' \
                            ...
                            '"Accidents caused by non-vehicles", "Accidents caused by vehicles", ' \
                            ...
                            '"Unknown disease that caused the death"]'

        self.level3_pass = ["Declared dead by the court","Intentional suicide","Homicide","Unknown accident","Diseases that cannot be classified as tumors or a specific system","Unknown disease that caused the death"]

        self.cause_level3_dict = {"Accidents caused by non-vehicles": '["Falling down and falling","Inanimate mechanical force","With life mechanical force","Drowning","Suffocation","Fire","Scalding","Electric current, radiation and extreme environment","Natural disasters","Toxic plants and animals","Toxic chemicals","Death from physical overexertion","Death from accidents during medical treatment"]',
                                  "Accidents caused by vehicles": '["Killed by vehicles while walking","Non-motorized vehicles accident","Two-wheeled motor vehicles accident","Three-wheeled motor vehicles accident","Car accident","Passenger car accident","Lorry accident","special vehicle accident","Rail accident","Ship accident","Aircraft accident"]',
                                  "Tumor": '["Malignant Tumors","Benign brain tumors","Hemangioma or aneurysm","Other benign tumors"]',
                                  "Diseases of the circulatory and blood system": '["Acute myocardial infarction","Cerebral Stroke","Coronary heart disease","Aortic disease","Heart valve disease","Heart disease of pulmonary origin","Heart Inflammation","Aplastic anemia","Other circulatory and blood disorders"]',
                                  ...
                                  "Diseases of the respiratory system": '["Acute respiratory failure, influenza, acute pneumonia","Chronic respiratory failure, chronic obstructive pulmonary disease, asthma, pulmonary fibrosis progression of respiratory failure"]',
...
def read_data(args):
    ...
    if args.pred_cause_level == 3:
        namespaces[f"data_claim_death_cause_level2"] = pd.read_csv(
            os.path.join(args.save_dir, 'Classify_causelevel2_final_res.csv'))[args.chunk_id::args.chunk_size]
    return namespaces
